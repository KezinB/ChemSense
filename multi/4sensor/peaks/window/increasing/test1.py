...
for i, sensor in enumerate(valid_sensors):
    ...
    for j in range(len(data) - window_size):
        ...
        if np.all(np.diff(window) > 0):  # Check if all differences are positive
            increasing_trends.append(j + window_size // 2)  # Use the middle point of the window
    ...
    plt.plot(time.iloc[increasing_trends], data.iloc[increasing_trends], "ro", label="Increasing Trends")  # Mark increasing trends
