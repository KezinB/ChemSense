...
for i, sensor in enumerate(valid_sensors):
    ...
    # Transform the data for better fitting
    y_log = np.log(data - data.min() + 1)
    
    # Fit an exponential decay trend line
    try:
        popt, pcov = curve_fit(exp_decay, time.index, y_log, p0=(1, 1e-6, 1), maxfev=2000)
        trend_line = exp_decay(time.index, *popt)
        trend_line = np.exp(trend_line) + data.min() - 1
        plt.plot(time, trend_line, label='Trend Line (Exp Decay)', color='magenta')
    except RuntimeError as e:
        print(f"Error fitting exponential decay for sensor {sensor}: {e}")
    ...
    print(f"Number of peaks for {sensor}: {len(upper_values)}")
