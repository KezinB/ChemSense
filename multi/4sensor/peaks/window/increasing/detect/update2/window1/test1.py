...
for i, sensor in enumerate(valid_sensors):
    ...
    # Window-based Decrease and Stabilization Detection
    window_size_decrease = 5  # Window size for decrease detection
    window_size_stabilize = 15  # Window size for stabilization detection
    decrease_threshold = -0.0005  # Threshold for decrease detection
    stabilization_variation = 0.0001  # Max variation allowed for stabilization

    for peak_time, peak_value in key_points[sensor]["Peak"]:
        peak_idx = time[time == peak_time].index[0]

        # Detect decrease using a windowed approach
        decrease_idx = peak_idx
        for j in range(peak_idx, len(data) - window_size_decrease):
            window = data[j:j + window_size_decrease]
            if np.mean(np.diff(window)) < decrease_threshold:
                decrease_idx = j
                break
        ...
        # Detect stabilization using a moving window
        ...
        for j in range(decrease_idx, len(data) - window_size_stabilize):
            ...
            if np.max(window) - np.min(window) < stabilization_variation:
                stability_idx = j
                break
