...
for i, sensor in enumerate(valid_sensors):
    ...
    # Fit a trend line
    slope, intercept, r_value, p_value, std_err = stats.linregress(time.index, data)
    trend_line = intercept + slope * time.index
    plt.plot(time, trend_line, label='Trend Line', color='magenta')
