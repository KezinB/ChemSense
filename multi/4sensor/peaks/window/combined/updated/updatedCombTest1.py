...
for i, sensor in enumerate(valid_sensors):
    ...
    upper_values = [max(segment) for segment in decreasing_segments]

    # Plot original data and mark upper and lower values of decreasing segments
    plt.subplot(len(valid_sensors), 1, i + 1)
    plt.plot(time, data, label=sensor, color='blue')
    
    for idx, (indices, segment, upper, lower) in enumerate(zip(segment_indices, decreasing_segments, upper_values, lower_values)):
        start, end = indices
